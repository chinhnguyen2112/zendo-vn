                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="box_search_forrm">
                            ...
                            <select name="" id="filter_u_apply">
                                <option value="0">Chọn người viết</option>
                                <?php
                                $list_u = $CI->Same->get_user();
                                if ($list_u != null) {
                                    foreach ($list_u as $val) {
                                ?>
                                        <option <?= ($this->input->get('user') == $val['id']) ? 'selected' : '' ?> value="<?= $val['id'] ?>"><?= $val['name'] ?></option>
                                <?php }
                                } ?>
                            </select>
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped">
                                ...
                                <tbody> <?php foreach ($feedback as $key => $val) { ?> <tr>
                                            ...
                                            <td><?= $val['name_apply'] ?></td>
                                            <td><?= $val['email'] ?></td>
                                            <td><?= $val['name_user'] ?></td>
                                            <td><?= date('d-m-Y H:i:s', $val['created_at']) ?></td>
                                            <td><?php
                                                echo '<a href="' . $val['link'] . '" target="_blank">' . (($val['link'] != '') ? 'Xem' : '') . '</a>';
                                                ?></td>
                                            <td><?= $val['text'] ?></td>
                                            <td>
...
<script>
    function filter_ds() {
        ...
        var u_apply = $('#filter_u_apply').val();
        var type = $('#filter_types').val();
        var error = $('#filter_error').val();
        var url = '/thong-ke-loi?keyword=' + keyword + '&project=' + project + '&duyet=' + duyet + '&user=' + u_apply + '&type=' + type + '&error=' + error
